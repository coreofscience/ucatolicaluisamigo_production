...
```{r}
model_1 <- 
  lm(articulos ~ dias_vinculados, researcher_model_1)
...
```{r}
researchres_split <- 
  initial_split(researcher_model_1, 
                prop = 0.75, 
                strata = articulos)
...
lm_fit <- lm_model |> 
  fit(articulos ~ dias_vinculados, researchers_training)
```
...
```{r}
researchers_test_results |> 
  rmse(truth = articulos, estimate = .pred)
```
...
```{r}
researchers_test_results |> 
  rsq(truth = articulos, estimate = .pred)
```
